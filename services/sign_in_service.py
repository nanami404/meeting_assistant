...
class SignInService(object):
    async def get_people_sign_status(self, db: AsyncSession, meeting_id: str) -> list[PersonSign]:
        """查询所有人员的签到状态（从数据库）"""
        # 可添加排序、过滤等逻辑（如按姓名排序）
    ...
    async def sign_person(self, db: AsyncSession, name: str, meeting_id: str, user_id: str) -> dict[str, str]:
        ...
        user_meeting_sign = result.scalar_one_or_none()
    ...
    async def leave_person(self, db: AsyncSession, name: str, meeting_id: str, user_id: str) -> dict[str, str]:
        """
        处理指定会议的人员请假逻辑
        :param db: 数据库会话
        :param name: 人员姓名
