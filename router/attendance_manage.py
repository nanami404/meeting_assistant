...
async def get_people_sign_status(db: Session = Depends(get_db)) -> List[PersonSignResponse]:
    """获取所有人员的签到状态"""
    try:
        # 调用服务层方法，传入数据库会话
        people = await meeting_service.get_people_sign_status(db)
        return people
...
async def sign(
    ...
    try:
        ...
        result = await meeting_service.sign_person(db, name, meeting_id,user_id)
        return result
...
async def leave(
    ...
    try:
        ...
        result = await meeting_service.leave_person(
            db=db,
            name=name,
